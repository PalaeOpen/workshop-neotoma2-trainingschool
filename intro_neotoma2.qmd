...
pacman::p_load(neotoma2, tidyverse, ggplot2, janitor)
```
...
Another classifier of the taxon list in Neotoma is the `ecologicalgroup`. In Tilia this is shown in the column *Group*. The function `get_table()` returns a table for all the existing table names in Neotoma. For instance, below is the list found in `ecologicalgroup`.

```{r}
neotoma2::get_table("ecolgroups")
```
...
## Excercise #2
Using the `pra_samples` dataset, write a tidyverse pipeline to calculate the relative proportions of aquatic indicators relative to all the entire sample counts (i.e., terrestrial).
