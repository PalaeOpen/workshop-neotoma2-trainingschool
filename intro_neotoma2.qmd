...
We can also be more restrictive and add extra arguments in `get_sites()` and look for pollen datasets and certain site names if we know what site we are looking for, or if we only know part of the name e.g. that is called lake in Czech: Jezero. 

```{r}
ezero_pollen <- get_sites(datasettype="pollen", loc=europe$geoJSON, sitename = "%Jezero%") #Note that % is a wildcard character
...
datasets(pra_data) # provides an overview of the datasets aailable for that site
```
