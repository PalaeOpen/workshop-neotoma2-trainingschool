...
subtitle: "PalaeOpen Training School — Sofia, 16 June 2026"
...
This workshop is designed to provide participants at the PalaeOpen Training School (Sofia, 15–19 June 2026) with an overview of the `neotoma2` R package and its use in obtaining and analyzing data from the Neotoma Paleoecology Database.
...
5.  Extract and inspect pollen datasets from the target lake site
...
This workflow relies on different R libraries to handle geographic data and interactive mapping. To streamline the setup, we use the `pacman` package. If any required package is missing from your local environment, `pacman` will automatically download and install it before loading.
...
-   Taxa
-   Counts
-   Stratigraphic plots -->  more in day 3 Thursday 18/07/26 [link](https://github.com/PalaeOpen/workshop-neotoma2-stratplots-trainingschool)
-   Chronologies --> see Thomas' age-depth modeling workshop [link](https://github.com/PalaeOpen/workshop_age-depth_trainingschool)

## Neotoma UML diagram {.tabset}

![Neotoma R Package UML diagram.](images/neotoma_uml.svg) 
...
For that, we will use the site [**Prášilské jezero**](https://data.neotomadb.org/47518), a mountain lake located in Czechia (Šumava Mts.). This site has been analyzed for several proxies, including pollen, diatoms, chironomids, charcoal, and geochronology.
...
neotoma2::summary(pra_data)
pra_samples <- samples(pra_data)
names(pra_samples)
```

The resulting `data.frame` has 13735 rows and 38 variables. The reason is because the table is in **long** format. The opposite format is **wide** format, with samples as rows, and variables as columns; the latter format is preferred for certain community analysis. But for now we continue with the **long** format.
...
```
Generally, the `collectionunits` are the different cores that were taken and each can have several `datasettypes`. For some `datasettypes`, or types of analyses some of columns stored in `samples()` may not be needed, however, for other dataset types they may be important. The `neotoma2` package includes as many column names as possible to cover a wide range of community/ proxy needs.

If you want to know what taxa we have in the record, you can use the function `taxa()` on the downloaded site object. This is basically the samples table with information about how many samples the taxon occurs, and in how many sites (in this case only one because we have filtered datasets and samples for *Prášilské jezero*). The `taxonid` values can be linked to the `taxonid` column in the `samples()`. This allows us to build taxon harmonization tables. However, this is not the focus of this introductory part.
...
# Working with counts
In many Neotoma datasets there exist samples that include different elements of analyses for the same taxon. For instance, in pollen datasets, it is common to find `element=pollen` and `element=stomate` for Pine. If we filter the taxa by `element=="pollen"` we might miss all the spores including *ferns* and *mosses*

![Tilia file Prášilské jezero, pollen dataset](images/tilia_screenshoot.png)
A very useful classifier of the taxon list in Neotoma is the `ecologicalgroup`. In Tilia this is shown in the column *Group*. The function `get_table()` can be used to pull existing table names in Neotoma. For instance, to see description of ecological groups: `neotoma2::get_table("ecolgrouptypes", limit = 100)`
...
```
We are interested in pollen and spores, including ferns and mosses. So we filter pollen samples first by `ecologicalgroup %in% c("TRSH", "UPHE", "VACR")`
...
These group codes allow us to ignore things that might skew our pollen sums—like algae (AQUA) or fungal spores (FUNG); even some pollen samples can also include dinoflagellate cysts (DINO). This way we focus on these groups to exclude certain taxa that do no represent the terrestrial vegetation in the final pollen sum.
...
pra_pollen_prop <- pra_pollen_counts %>%
  ...
  dplyr::select(depth, age, variablename, prop, ecologicalgroup) %>% #select only relevant columns
  dplyr::mutate(prop = as.numeric(prop)) #make sure proportions are numeric

```

## Excercise #2
Using the `pra_samples` object, calculate the relative proportions of aquatic pollen taxa relative to all the entire sample counts. 
...
| *tidypaleo* | long format | `ggplot()`|
| *analogue* | wide format | `Stratiplot()`|
...
If you are happy and want to save the resulting plot:
```{r}
#| eval: false
#| include: true
pdf("pollen_plot.pdf", width = 10, height = 8)
...
# Conclusion
We have covered different steps to query the Neotoma Palaeoecological Database and get samples from the selected dataset to perform a basic stratigraphic plot. We hope you can use this example workflow as a template for your own work.
